...
from dsets import get_dsets_remove_class, get_dsets_VGGFace_10_subjects
from utils import accuracy, compute_metrics, set_seed, compute_losses, simple_mia, get_retrained_model,get_resnet50_trained_on_VGGFace_10_subjects,get_outputs
...
def main():
    ...
    ##### GET DATA #####
    train_loader, test_loader, forget_loader, retain_loader, test_loader_retain,test_loader_forget = get_dsets_VGGFace_10_subjects()
    print(f'batch forget loader: {len(forget_loader)}')
    ...
    print(f"TEST-LOADER of retain:{accuracy(original_pretr_model, test_loader_retain):.3f} \n")
    ...
    print(f"TEST-LOADER:{accuracy(unlearned_model, test_loader_retain):.3f} \nFORGET-LOADER: {accuracy(unlearned_model, test_loader_forget):.3f}\nRETAIN-LOADER: {accuracy(unlearned_model, retain_loader):.3f}  ")
