...
print(X.shape,classes.shape,classes2.shape)
...
features_un = logits_all_unlr_fgt.numpy()
features_un2 = logits_all_unlr_ret.numpy()
print(features_un.shape,features_un2.shape)
X =np.concatenate((X,features_un[cls_unlr_fgt<10][::N,:],features_un2[cls_unlr_ret<10][::N,:]),axis=0)

classes_un = cls_unlr_fgt[cls_unlr_fgt<10][::N]
classes_un2 = cls_unlr_ret[cls_unlr_ret<10][::N]

print(X.shape, classes_un.shape,classes_un2.shape)

mod = TSNE(n_components=2, learning_rate='auto',init='random', perplexity=3)

X_transf = mod.fit_transform(X)

X_emb = X_transf[:classes.shape[0],:]
X_emb2 = X_transf[classes.shape[0]:classes.shape[0]+classes2.shape[0],:]
X_emb_un = X_transf[classes.shape[0]+classes2.shape[0]:classes.shape[0]+classes2.shape[0]+classes_un.shape[0],:]
X_emb_un2 = X_transf[classes.shape[0]+classes2.shape[0]+classes_un.shape[0]:,:]

color = ['k','red','green','gold','pink','violet','orange','aqua','blue','gainsboro']
for i in range(0,10):
    ax[0,0].scatter(X_emb[classes==i,0],X_emb[classes==i,1],c=color[i],s=3)

    ax[1,0].scatter(X_emb2[classes2==i,0],X_emb2[classes2==i,1],c=color[i],s=3)
    ax[0,1].scatter(X_emb_un[classes_un==i,0],X_emb_un[classes_un==i,1],c=color[i],s=3)
    ax[1,1].scatter(X_emb_un2[classes_un2==i,0],X_emb_un2[classes_un2==i,1],c=color[i],s=3)

ax[0,0].set_title('Original model fgtset')
ax[1,0].set_title('Original model test set')

ax[0,1].set_title('UNlearned fgtset')
ax[1,1].set_title('UNlearned test set')

plt.savefig(f'tsne_fgt_unlr.png')
plt.close()