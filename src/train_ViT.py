...
def trainer(removed=None):
    ...
    net = ViT_16_mod(n_classes=200).to(opt.device)
    #load weights
    ...
    optimizer = optim.Adam(net.parameters(), lr=0.0005, weight_decay=5e-5,eps=1e-8,betas=(0.9,0.999))
    train_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs*len(trainloader))
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(net.state_dict(), f'/home/jb/Documents/MachineUnlearning/src/weights/chks_{opt.dataset}/best_checkpoint_ViT.pth')
        net.train()
