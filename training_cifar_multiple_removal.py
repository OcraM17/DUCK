...
def compute_mean_std(trainloader):
    ...
    """
    for i, inp in enumerate(trainloader, 0): 
        print(i)   
        data, labels = inp
        ...
        if i==0:
            ...
            data_r = torch.cat((data_r, data[:,0,:,:]), axis=0)
            data_g = torch.cat((data_g, data[:,1,:,:]), axis=0)
            data_b = torch.cat((data_b, data[:,2,:,:]), axis=0)
...
def trainer(class_to_be_r,seed):
    ...
    trainsettt = torchvision.datasets.CIFAR100(root='~/data', train=True, download=True, transform=transform_tt)
    forget_set, retain_set = split_retain_forget(trainsettt, class_range[:class_to_be_r])
    ###normaliz
    ...
    net = torchvision.models.resnet18(pretrained=True).to('cuda:1')    
    net.conv1 = nn.Conv2d(3, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False).to('cuda:1')
    net.maxpool = nn.Identity()
    #net.fc = nn.Sequential(nn.Dropout(0.4), nn.Linear(512, 10)).to('cuda:1')
    net.fc = nn.Sequential(nn.Dropout(0), nn.Linear(512, 100)).to('cuda:1')

    #load weight to network
    net.load_state_dict(torch.load('/home/jb/Documents/MachineUnlearning/weights/Final_CIFAR100_Resnet18.pth'))
...
if __name__ == '__main__':
    ...
    acc_in = []
    for i in [i*10 for i in range(9,10)]+[98]:
        print('eeeeeeeeeeeeeee',i)
